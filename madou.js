var rule = {
    ...
    limit: 6,
    推荐: '',
    "一级": "js: try { var items = []; var html = request(input); if (!html) { items.push({title: '⚠️ 网页请求超时', img: '', desc: '网络问题', url: input}); } else if (html.indexOf('Cloudflare') !== -1 || html.indexOf('Just a moment') !== -1) { items.push({title: '⚠️ 被网站防火墙(CF盾)拦截', img: '', desc: '请更换节点', url: input}); } else { var list = pdfa(html, '.cardBox, .section-content__item, .video-card, .list-item, div[class*=\"item\"]'); if (list.length === 0) { items.push({title: '⚠️ 找到0个视频', img: '', desc: '网页改版或空分类', url: input}); } else { list.forEach(function(it) { var title = pdfh(it, '.cardTitle&&Text') || pdfh(it, 'img&&alt') || pdfh(it, 'a&&title'); var img = pd(it, 'img&&data-src') || pd(it, 'img&&src'); var url = pd(it, 'a&&href'); if (title && url) { items.push({ title: title.trim(), img: img, desc: pdfh(it, '.cardTag&&Text') || 'HD', url: url }); } }); } } setResult(items); } catch(e) { setResult([{title: '❌ 解析出错: ' + e.message, img: '', desc: '代码异常', url: input}]); }",
    二级: {
        "title": "h1&&Text",
